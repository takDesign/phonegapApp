...
navigator.geolocation.getCurrentPosition(
    function(position) {
        ...
        $("#save").bind("click", async function() {
            await insertPlace(
                $(
                    $("#placeName"),
                    position.coords.latitude,
        ...
        function insertPlace(name, lat = "", lon = "") {
            // Let's create a database
            db = openDatabase("newdb", "1", "My New DB", dbSize);

            db.transaction(function(tx) {
                tx.executeSql(
                    "CREATE TABLE IF NOT EXISTS " +
                        "PLACE(ID INTEGER PRIMARY KEY ASC, strName, strLat, strLon)"
                );
                tx.executeSql(
                    "INSERT INTO PLACE " +
                        "(strName, strLat, strLon) values(?,?,?)",
                    [name, lat, lon]
                );
            });
        }
